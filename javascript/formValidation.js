...
function recieptFuncCaller(event) {
  ...
  // mastery requirement: store validator result
  let isValid = validatorFunc();
  ...
  if (isValid == true) {
    ...
    // required to store calculation before logging
    let totalCost = calculationFunc();
    ...
    console.log("The total of your order is: $" + totalCost);
  } else {
    return;
  }
}
...
function validatorFunc() {
  ...
  if (firstName.value === "") {
    alert("Missing input. Please enter your first name.");
    firstName.focus();
    return false;
  }

  if (lastName.value === "") {
    alert("Missing input. Please enter your last name.");
    lastName.focus();
    return false;
  }
